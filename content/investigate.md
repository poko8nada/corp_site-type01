...
> 実在の公式ドメイン・電話・SNS・アクセス解析の測定ID・具体URLはデモ用に架空化しています。技術スタックの型、ページ構成の型、サードパーティ連携の型は、元の調査観察を踏まえた一般化です。`pre_survey_casual_bar_Lyra.md` と同一のデモ世界観（中洲エリアのカジュアルバー案件想定）に揃えています。
